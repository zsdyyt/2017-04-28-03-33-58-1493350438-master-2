			...
			if (str2.charAt(i) == 'X') {
				if (i < str2.length() - 2) {
					if (str2.charAt(i + 2) == 'X') {
						...
						if (i < str2.length() - 4) {
							if (str2.charAt(i + 3) == 'X') {
								...
								if (str2.charAt(i + 3) == '/') {
									...
									Score = 10 + Character.getNumericValue(str2.charAt(i + 3))
											+ Character.getNumericValue(str2.charAt(i + 4)) + Score;
								}
							}
						}
					}
						...
						if (i < str2.length() - 3 && str2.charAt(i + 3) == '/') {
							...
							Score = 10 + Character.getNumericValue(str2.charAt(i + 2))
									+ Character.getNumericValue(str2.charAt(i + 3)) + Score;
						}
					}
				...
				if (i < str2.length() - 2 && str2.charAt(i + 2) == 'X') {
					...
					if (str2.charAt(i + 3) == '-') {
						...
						Score = 10 - Character.getNumericValue(str2.charAt(i - 1))
								+ Character.getNumericValue(str2.charAt(i + 3)) + Score;
					}

				} else {
					Score = 10 - Character.getNumericValue(str2.charAt(i - 1))
							+ Character.getNumericValue(str2.charAt(i + 2)) + Score;
				}
